...
class TestSimpleCrowding:
    def test_no_logistic_returns_one(self):
        ...
        assert strategy.crowding(make_clone(), t=0.0, numerator_N=50) == 1.0

    def test_empty_population_returns_one(self):
        cfg = simple_config(K0=100.0)
        strategy = SimpleCrowding(cfg)
        assert strategy.crowding(make_clone(), t=0.0, numerator_N=0) == 1.0

    def test_at_capacity_returns_zero(self):
        cfg = simple_config(K0=100.0)
        strategy = SimpleCrowding(cfg)
        result = strategy.crowding(make_clone(), t=0.0, numerator_N=100)
        assert result == pytest.approx(0.0)

    def test_above_capacity_clamped_to_zero(self):
        cfg = simple_config(K0=100.0)
        strategy = SimpleCrowding(cfg)
        result = strategy.crowding(make_clone(), t=0.0, numerator_N=150)
        assert result == 0.0

    def test_crowding_between_zero_and_one(self):
        cfg = simple_config(K0=100.0)
        strategy = SimpleCrowding(cfg)
        result = strategy.crowding(make_clone(), t=0.0, numerator_N=50)
    ...
    def test_K_declines_with_time(self):
        ...
        strategy = SimpleCrowding(cfg)
        c0 = strategy.crowding(make_clone(), t=0.0, numerator_N=50)
        c10 = strategy.crowding(make_clone(), t=10.0, numerator_N=50)
    ...
    def test_K_never_below_Kmin(self):
        ...
        result = strategy.crowding(make_clone(), t=1000.0, numerator_N=5)
        assert result >= 0.0
...
class TestAdaptedCrowding:
    ...
    def test_result_is_non_negative(self):
        cfg = simple_config(K0=100.0)
        strategy = AdaptedCrowding(cfg)
        result = strategy.crowding(make_clone(birth_rate=0.5, death_rate=0.2), t=0.0, numerator_N=200)
        assert result >= 0.0

    def test_result_at_most_one(self):
        cfg = simple_config(K0=100.0)
        strategy = AdaptedCrowding(cfg)
        result = strategy.crowding(make_clone(birth_rate=0.5, death_rate=0.2), t=0.0, numerator_N=1)
        assert result <= 1.0
    ...
    def test_higher_fitness_yields_higher_effective_birth_rate(self):
        ...
        rb_low  = c_low.birth_rate_effective()
        rb_high = c_high.birth_rate_effective()
