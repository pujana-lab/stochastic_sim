...
class TestRateMatrix:
    ...
    def test_add_event(self):
        rm = RateMatrix()
        e = Event(kind=EventType.BIRTH, clone_id=(0,), rate=0.5)
        rm.add_event(e)
    ...
    def test_add_multiple_events(self):
        rm = RateMatrix()
        events = [
            Event(kind=EventType.BIRTH, clone_id=(0,), rate=0.5),
            Event(kind=EventType.DEATH, clone_id=(1,), rate=0.2),
            Event(kind=EventType.MUTATION, clone_id=(0, 1), rate=0.01),
        ]
        for e in events:
            rm.add_event(e)
    ...
    def test_clear_events(self):
        rm = RateMatrix()
        rm.add_event(Event(kind=EventType.BIRTH, clone_id=(0,), rate=0.5))
        rm.add_event(Event(kind=EventType.DEATH, clone_id=(1,), rate=0.2))
        rm.clear()
        assert len(rm.events) == 0
    ...
    def test_choose_event(self):
        ...
        rm = RateMatrix()
        e1 = Event(kind=EventType.BIRTH,    clone_id=(0,),   rate=0.5)
        e2 = Event(kind=EventType.DEATH,    clone_id=(1,),   rate=0.2)
        e3 = Event(kind=EventType.MUTATION, clone_id=(0, 1), rate=0.3)
        ...
        rm2 = RateMatrix()
        f1 = Event(kind=EventType.BIRTH,    clone_id=(0,),   rate=0.50)
        f2 = Event(kind=EventType.DEATH,    clone_id=(1,),   rate=0.25)
        f3 = Event(kind=EventType.MUTATION, clone_id=(0, 1), rate=0.10)
