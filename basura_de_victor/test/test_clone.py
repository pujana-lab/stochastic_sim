...
def test_die_decrements_N():
    c = make_clone(N=5)
    c.die()
    assert c.N == 4

def test_die_does_not_go_below_zero():
    c = make_clone(N=0)
    c.die()
    assert c.N == 0
...
def test_mutated_child_decrements_parent_N():
    c = make_clone(N=5)
    c.mutated_child(fitness_gain=0.1, instability_jump=0.05, buildup_gain=0.0)
    assert c.N == 4

def test_mutated_child_N_is_one():
    c = make_clone(N=5)
    child = c.mutated_child(fitness_gain=0.1, instability_jump=0.0, buildup_gain=0.0)
    assert child.N == 1
...
def test_mutated_child_inherits_death_rate():
    ...
    child = c.mutated_child(fitness_gain=0.0, instability_jump=0.0, buildup_gain=0.0)
    assert child.death_rate == pytest.approx(0.3)

def test_mutated_child_raises_on_dead_clone():
    c = make_clone(N=0)
    with pytest.raises(ValueError):
        c.mutated_child(fitness_gain=0.1, instability_jump=0.0, buildup_gain=0.0)

def test_mutated_child_id_increments():
    c = make_clone(clone_id=(), N=5)
    child1 = c.mutated_child(0.0, 0.0, 0.0)
    c.N += 1  # restore so we can mutate again
    child2 = c.mutated_child(0.0, 0.0, 0.0)
